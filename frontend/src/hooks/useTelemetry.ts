...
export default function useTelemetry() {
  ...
  useEffect(() => {
    ...
    const handleClick = () => {
      ...
      if (rageClick) telemetryRef.current.rageClicks++;
      updateTelemetry();
    };
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "Backspace") telemetryRef.current.backspaces++;
      updateTelemetry();
    };
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      telemetryRef.current.formDuration = Math.floor(
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const response = await sendTelemetry(telemetryRef.current);
        if (response?.adaptiveUI) {
          // Dispatch event for Adaptive UI
